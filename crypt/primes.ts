...
function eulerTotient(n: number): boolean {
  let phi = n;
...
function primeFactorialDivision(n: number): number[] {
  const primes = [];
  let i = 2;
  while (i <= n) {
    if (n % i === 0) {
...
/// x^exp % mod
export function squareAndMultiply(x: number, exp: number, mod: number): number {
...
export function primitiveRoot(n: number): number {
  ...
  for (let i = 2; i < n; i++) {
    ...
    if (completed === phiDiv.length) return i;
  }
  return 0;
}
